...
describe("DomainEvents", () => {
    describe("One publisher", () => {
        ...
        test("should receive only filtered events", () => {
            publisher.subscribe(
                subscriber,
                DomainEvent,
                (event: DomainEvent) => event.timeMsec == 13
            );
            publisher.publish(new DomainEvent(13));
            expect(subscriber.handleEvent.mock.calls.length).toBe(1);
            publisher.publish(new DomainEvent(14));
            expect(subscriber.handleEvent.mock.calls.length).toBe(1);
        });
        ...
        test("should not allow to subscribe for child class", () => {
            publisher.subscribe(subscriber);
            expect(() => publisher.subscribe(subscriber, MyEvent)).toThrow(
                DomainEventError
            );
        });
        ...
        test("should not allow to subscribe for parent class", () => {
            publisher.subscribe(subscriber, MyEvent);
            expect(() => publisher.subscribe(subscriber)).toThrow(
                DomainEventError
            );
        });
    });
